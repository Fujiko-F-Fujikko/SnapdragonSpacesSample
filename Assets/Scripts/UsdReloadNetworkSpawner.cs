...
public class UsdReloadNetworkSpawner : MonoBehaviour
{
    ...
    [Tooltip("Spawnableとして扱うオブジェクトのキーワードリスト。マッチした順に kind=10+index を割り当てます")]
    ...
    private void OnUsdReloaded(UsdAsset asset)
    {
        ...
        {
            if (verboseLog) Debug.Log("[USD-Net] Skip spawn (not server).", this);
            return;
        }
        DespawnAll();
        SpawnFromUsd();
    ...
    private void SpawnFromUsd()
    {
        ...
        {
            if (verboseLog) Debug.LogWarning("[USD-Net] dummyNetworkObjectPrefab が設定されていません", this);
            return;
        }
        ...
        foreach (var tr in root.GetComponentsInChildren<Transform>(true))
        {
            ...
            {
                Debug.LogWarning("[USD-Net] Prefab に NetworkObject / UsdDummyView がありません", dummyNetworkObjectPrefab);
                Destroy(go);
                continue;
            }
            ...
            if (verboseLog)
            {
                Debug.Log($"[USD-Net] Spawn dummy kind={kind} path={usdPath}", go);
            }
        }
    }
...
public class UsdReloadNetworkSpawner : MonoBehaviour
{
  ...
  void Awake() { if (logOnce) Debug.Log("[USD-Net] UsdReloadNetworkSpawner disabled (HAS_UNITY_USD not defined).", this); }
}
#endif
